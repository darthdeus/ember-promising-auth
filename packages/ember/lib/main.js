// Version: v1.0.0-rc.3-318-g5ec0abd
// Last commit: 5ec0abd (2013-05-25 20:09:33 -0400)
...
define("router",
  ...
  function(RouteRecognizer, RSVP) {
    ...
    */
    ...
    Router.TransitionRedirected = function(message) {
      this.message = (message || ""); 
      this.name = "TransitionRedirected";
    };
    ...
    Router.prototype = {
      ...
      */
      handleURL: function(url) {
        ...
        log(this, "Attempting URL transition to " + url);

        if (!results) {
          return RSVP.reject(new Router.UnrecognizedURLError(url));
        }

        // Construct the array of HandlerInfos that
        // `performTransition` expects.
        ...
        return performTransition(this, handlerInfos, 0, {});
      },
      ...
      */
      transitionTo: function(name) {
        ...
        return namedTransition(this, name, this.updateURL, args);
      },
      ...
      */
      replaceWith: function(name) {
        var args = slice.call(arguments, 1);
        return namedTransition(this, name, this.replaceURL, args);
      },
    ...
    function getMatchPoint(handlers, objects) {
      var i, objectsToMatch = objects.length, startIdx = handlers.length;
      ...
      if (objectsToMatch > 0) {
        var handlerName = handlers[handlers.length - 1].handler;
    ...
    */
    function paramsForHandler(router, handlerName, objects) {
      ...
      var handlers = router.recognizer.handlersFor(handlerName),
          params = {},
          startIdx = getMatchPoint(handlers, objects),
    ...
    /**
      @private
    */
    function namedTransition(router, handlerName, urlMethod, providedObjects) {

      var resolvedParentContexts = {},
          handlers = router.recognizer.handlersFor(handlerName),
          handlerInfos = [],
          ancestorMatchPoint = 0,
          handlerObj, handlerInfo, name, handler;

      log(router, "Attempting transition to " + handlerName + ", # of contexts: " + providedObjects.length);

      // Check if this route's already been entered. isActive will check
      // resolved contexts. We'll also have to later check for duplicate no-op
      // transitionTo's in the middle of a transition's validations.
      if (router.isActive.apply(router, [handlerName].concat(providedObjects))) {
        log(router, handlerName + " is already active. Marking transition as successful");
        return RSVP.resolve(router.targetHandlerInfos);
      }

      // Find common ancestor match point.
      var currentHandlerInfos = router.currentHandlerInfos || [];
      for (var len = currentHandlerInfos.length; ancestorMatchPoint < len; ancestorMatchPoint++) {
        handlerInfo = currentHandlerInfos[ancestorMatchPoint];
        handlerObj = handlers[ancestorMatchPoint];
        if (!(handlerInfo && handlerObj && handlerObj.name === handlerInfo.name)) {
          break;
        }
      }

      var matchPoint = Math.min(ancestorMatchPoint, getMatchPoint(handlers, providedObjects)),
          previousHandlerInfos = router._lastNamedTransitionHandlerInfos,
      ...
      for (var i = handlers.length - 1; i >= 0; i--) {
        ...
        if (i >= matchPoint) {
          // If this handler is dynamic and we have objects to provide it...
          if (handlerInfo.isDynamic && providedObjects.length) {
            handlerInfo.providedContext = providedObjects.pop();
          } else if (providedObjects.length) {
            handlerInfo.fetchNewModel = true;
          }
        } else {
          // This handler isn't being entered or exited.
          // Just store its context in the resolvedParentContexts hash
          // so that leafier routes will have access to it during validation.
          handlerInfo.context = resolvedParentContexts[name] = handler.context;
        }

        // Check if there's an in-process
        var previousHandlerInfo = isDuplicateTransition && previousHandlerInfos && previousHandlerInfos[i];
        if (previousHandlerInfo) {
          var currentContext  = handlerInfo.providedContext || handlerInfo.context,
              previousContext = previousHandlerInfo.providedContext || previousHandlerInfo.context;

          if (!(handlerInfo.name === previousHandlerInfo.name &&
                currentContext === previousContext)) {
            isDuplicateTransition = false;
          }
        } else {
          isDuplicateTransition = false;
        }

        handlerInfos.unshift(handlerInfo);
      }

      if (isDuplicateTransition) {
        log(router, "Transition to " + handlerName + " is already underway. Deferring to first transition attempt.");

        // This attempted transition is a duplicate of one that's already in progress.
        // Just return the promise of the in-progress transition so that anyone who
        // .then's to this one will resolve/reject the same way as the original.
        return router._lastNamedTransition;
      }

      router._lastNamedTransitionHandlerInfos = handlerInfos;

      return router._lastNamedTransition = 
        performTransition(router, handlerInfos, matchPoint, resolvedParentContexts, urlMethod);
    }
    ...
    */
    ...
    /**
      Returns a promise that resolves if the transition represented
      by this TransitionEvent would succeed. The resolve value
      is a commit function that, when called, will perform the
      validated transition.
     */
    function performTransition(router, handlerInfos, matchPoint, resolvedContexts, urlMethod) {

      // Bump the current transition sequence. This will
      // be used to determine whether another transition
      // has begun, signalling that any others in progress
      // should halt.
      router.transitionSequence += 1;
      var currentSequence = router.transitionSequence;

      log(router, currentSequence, 
          "Beginning validation for transition to " + handlerInfos[handlerInfos.length - 1].name);

      return resolveContexts(router, handlerInfos, matchPoint, resolvedContexts)
             .then(transitionSuccess);

      function transitionSuccess() {

        log(router, currentSequence, "Validation succeeded, running enter/setup/exit handlers.");

        router._lastNamedTransitionHandlerInfos = router._lastNamedTransition = null;
        updateURL(router, handlerInfos, urlMethod);
        setupContexts(router, handlerInfos);

        log(router, currentSequence, "TRANSITION COMPLETE.");

        return handlerInfos;
      }
    }

    function updateURL(router, handlerInfos, urlMethod) {
      if (!urlMethod) { return; }

      var objectsForDynamicHandlers = [];
      for (var i = 0, len = handlerInfos.length; i < len; ++i) {
        var handlerInfo = handlerInfos[i];
        if (handlerInfo.isDynamic) {
          objectsForDynamicHandlers.push(handlerInfo.context);
        }
      }

      var handlerName = handlerInfos[handlerInfos.length - 1].name,
          params = paramsForHandler(router, handlerName, objectsForDynamicHandlers),
          url = router.recognizer.generate(handlerName, params);

      urlMethod.call(router, url);
    }

    function resolveContexts(router, handlerInfos, index, resolvedContexts) {

      if (index === handlerInfos.length) {
        // No more contexts to resolve.
        return RSVP.resolve(resolvedContexts);
      }

      var currentSequence = router.transitionSequence,
          handlerInfo = handlerInfos[index],
          handler = handlerInfo.handler,
          contextHasFulfilled = false,
          errorAlreadyHandled = false,
          handlerName = handlerInfo.name;

      function checkForRedirect(value) {
        if (currentSequence !== router.transitionSequence) {
          log(router, currentSequence, handlerName + ": transition was redirected");
          return RSVP.reject(new Router.TransitionRedirected());
        }
        return value;
      }
      ...
      function handleError(reason) {
        ...
        if (reason instanceof Router.TransitionRedirected) {
          // Redirects don't fire error handler
          return RSVP.reject(reason);
        }

        log(router, currentSequence, handlerName + ": handling error: " + reason);

        // An error was thrown / promise rejected, so run
        // the default `error` handler if it exists.
        if (handler.error) { handler.error(reason); }
      ...
      function beforeModel() {
        log(router, currentSequence, handlerName + ": calling beforeModel hook");
        return RSVP.resolve(handler.beforeModel &&
                            handler.beforeModel(resolvedContexts));
      }

      function getContext() {
        var toFulfill;
        if (handlerInfo.hasOwnProperty('providedContext')) {

          log(router, currentSequence, handlerName + ": using provided model instead of calling model() hook");

          // This is a transitionTo with a provided context.
          toFulfill = handlerInfo.providedContext;
        } else {
          if (!handler.hasOwnProperty('context') || handlerInfo.fetchNewModel) {

            log(router, currentSequence, handlerName + ": calling .model()");

            delete handlerInfo.fetchNewModel;
            toFulfill = handler.model && handler.model(handlerInfo.params || {}, resolvedContexts);
          } else {

            log(router, currentSequence, handlerName + ": reusing existing handler model");
            toFulfill = handler.context;
          }
        }
        return RSVP.resolve(toFulfill);
      }
      ...
      function afterModel(context) {
        ...
        log(router, currentSequence, handlerName + ": calling afterModel hook");
        return RSVP.resolve(handler.afterModel && handler.afterModel(context, resolvedContexts))
      ...
      function proceed(context) {
        ...
        // resolvedContexts hash so that it's available to leafier
        // handlers while they're being validated.

        log(router, currentSequence, handlerName + " has been validated, advancing to next handler");

        handlerInfo.context = resolvedContexts[handlerInfo.name] = context;
        return resolveContexts(router, handlerInfos, index + 1, resolvedContexts);
      }
    ...
    function log(router, sequence, msg) {
      ...
      if (arguments.length === 3) {
        console.log("Transition #" + sequence + ": " + msg);
      } else {
        msg = sequence;
        console.log(msg);
      }
...
Ember.Router = Ember.Object.extend({
  ...
  routeToURL: function(url) {
    this.routeTo(Ember.TransitionEvent.transitionByURL(url));
  },
...
function doTransition(router, method, args) {
  ...
  return router.router[method].apply(router.router, args).then(function() {
...
Ember.Route = Ember.Object.extend({
  ...
  */
   ...
   */
  routeTo: function(transitionEvent) {
    return this.router.routeTo(transitionEvent);
  },
  ...
  */
  ...
  /**
    @private
  */
  beforeModel: function(resolvedParentModels) {
    return this.prepareTransition(resolvedParentModels);
  },

  prepareTransition: Ember.K,
  validateTransition: Ember.K,
  ...
  /**
    ...
    The router.js hook that is called after the model 
    has loaded (after any of its promises have resolved).
   */
  afterModel: function(model, resolvedParentModels) {
    if (this.redirect !== Ember.K) {
      Ember.deprecate('`redirect` is deprecated. Please put any redirect logic that should take place before the model has loaded in `prepareTransition`, and put any redirect logic that should take place after the model has loaded in `validateTransition`');
      return this.redirect(model, resolvedParentModels);
    } else {
      return this.validateTransition(model, resolvedParentModels);
    }
  },
...
Ember.URLTransitionEvent = Ember.TransitionEvent.extend({
  ...
  perform: function() {
    return this.router.handleURL(this.get('url'));
  }
});

Ember.NamedTransitionEvent = Ember.TransitionEvent.extend({
  transitionMethod: 'transitionTo',
  providedContexts: null,

  perform: function() {
    var router = this.router,
        args = [this.destinationRouteName].concat(this.providedContexts);

    return router[this.get('transitionMethod')].apply(router, args);
  }
});
...
Ember.TransitionEvent.reopenClass({
  ...
  transitionTo: function(routeName, contexts) {
    return Ember.NamedTransitionEvent.create({
      destinationRouteName: routeName,
      providedContexts: contexts
    });
  },

  replaceWith: function(routeName, contexts) {
    return Ember.NamedTransitionEvent.create({
      destinationRouteName: routeName,
  ...
  transitionByURL: function(url) {
    return Ember.URLTransitionEvent.create({ url: url });
  }
});
  ...
  var LinkView = Ember.LinkView = Ember.View.extend({
    ...
    click: function(event) {
      ...
      var routeArgs = args(this, router),
          ...
          transitionEvent = Ember.TransitionEvent[transitionMethod](routeArgs[0], routeArgs.slice(1));
      router.routeTo(transitionEvent);
    },
...
Ember.Application.reopenClass({
  ...
  */
  buildContainer: function(namespace) {
    ...
    //container.register('transition_event:basic', Ember.TransitionEvent, { instantiate: false });
    ...
    //container.injection('transition_event', 'router', 'router:main');
